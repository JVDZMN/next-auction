...
const STEPS: { id: number; label: string; desc: string; Icon: IconFC }[] = [
  {
    id: 1, label: 'Register', desc: 'Create your free account in minutes',
    Icon: ({ className }) => (
  ...
  {
    id: 2, label: 'Select', desc: 'Browse and find your dream vehicle',
    Icon: ({ className }) => (
      <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  {
    id: 3, label: 'Bid', desc: 'Place real-time competitive bids',
    Icon: ({ className }) => (
      <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  {
    id: 4, label: 'Win', desc: 'Secure your winning vehicle',
    Icon: ({ className }) => (
      <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <section className="min-h-screen flex flex-col justify-center py-16 sm:py-20" style={{ backgroundColor: 'var(--dark-section)' }}>
      <div className="mx-auto max-w-6xl px-6 sm:px-10">
        <motion.div initial={{ opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={SPX} className="mb-16 text-center">
          <p className="mb-3 text-xs font-bold uppercase tracking-[0.25em]" style={{ color: 'var(--copper)' }}>How It Works</p>
          <h2 className="text-3xl font-black sm:text-4xl" style={{ color: 'var(--text-light)' }}>Four Steps to Your Next Car</h2>
        </motion.div>
            ...
            <Fragment key={step.id}>
              ...
              >
                <motion.div
                  ...
                  whileHover={{ scale: 1.1, boxShadow: 'rgba(8,41,36,0.25) 0 0 0 10px' }}
                  transition={SP}
                >
